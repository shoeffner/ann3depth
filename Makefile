...
distributed: ${LOG_DIR} ./tools/grid/startup.sh
	MEMORY_RATIO=${MEMORY_RATIO} GRID_QUEUES=${GRID_QUEUES} GRID_PREFIX=${GRID_PREFIX} PS_NODES=${PS_NODES} WORKERS=${WORKERS} CONDAENV=${CONDAENV} MODEL=${MODEL} STEPS=${STEPS} BATCHSIZE=${BATCHSIZE} DATASET=${DATASET} CKPT_DIR=${CKPT_DIR} CKPT_FREQ=${CKPT_FREQ} SUM_FREQ=${SUM_FREQ} DATA_DIR=${DATA_DIR} TIMEOUT=${TIMEOUT} qsub ${GRID_QUEUE_ARG} -N ${GRID_PREFIX}_keepalive ./tools/grid/distributed_master.sge
